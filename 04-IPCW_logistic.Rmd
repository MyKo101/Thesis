...
```{r PM-DGM, echo=F}
tribble(~"Performance Measure",~"Estimand",~"SE",
        "Unweighted","$\\bar{Q}=\\frac{1}{100}\\sum_{j=1}^{100}\\hat{Q}(t_j)$","$\\bar{Q}_{SE}=\\sqrt{\\frac{1}{100}\\sum_{j=1}^{100} \\hat{Q}_{SE}(t_j)^2 }$",
  ...
  if_fun(get_format("gitbook"),
         function(x) mutate(x,
                            Estimand = gsub("$","$$",Estimand,fixed=T),
                            SE = gsub("$","$$",SE,fixed=T))) %>%
  kable(format=fb("html","latex","latex"),
        caption=paste0(fb("<font size=\"2\">","{\\small ","{\\small "),
                       "Performance Measures to be taken at each time point",
...
```{r MainPlot,echo=F, fig.caption="Bias, Coverage and Empirical Standard Error for the Over-estimating, Perfect and Under-Estimating models across all four methods. The dotted/dashed lines show the unweighted/weighted average across all time points"}
knitr:::include_graphics("figure/IPCW_Logistic/Main_Plot.png")
```
