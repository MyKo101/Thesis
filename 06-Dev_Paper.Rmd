...
<br>
```{r Continuous-Demo, echo=F}
"data/Dev_Paper_Continuous_Demographics.csv" %>%
  read_csv(col_types=cols()) %>%
  Dev_Valid_Paper_Var_Names(Variable,group_var = ".group") %>%
  rename(`SKS (Development)` = res_SKS,`SERPR (Validation)` = res_UoG) %>%
  mutate(Variable = paste0(Variable,
                           case_when(grepl("eGFR",Variable) ~ fm(1),
                                     grepl("BP",Variable) ~ fm(2),
                                     Variable == "BMI" ~ fm(3),
                                     Variable %in% c("Albumin","Haemoglobin") ~ fm(4),
                                     Variable %in% c("Phosphate","Corrected Calcium") ~ fm(5),
                                     grepl("uPCR",Variable) ~ fm(6),
                                     T~""))) %>%
  to_kable(caption="Population demographics for the continuous variables presented as: mean (IQR) [min,max] <number missing (percent missing)>",
           group_col=".group", align="rr", lscape=T,
           row_names="Variable",numeric_cols=2:3,
           col_widths=c("13em","33em","36em")) %>%
  footnote(alphabet=c("(ml/min/1.73m^2) or per year","(mmHG)","(kg/m^2)",
                      "(g/l)","(mmol/l)","(g/mmol) or per year"),
           footnote_as_chunk=T)
...
```{r Categorical-Demo, echo=FALSE}
"data/Dev_Paper_Categorical_Demographics.csv" %>%
  read_csv(col_types=cols()) %>%
  mutate(Variable = Variable) %>%
  fill(Variable) %>%
  mutate(Category = if_else(Category == "<Missing>",
                            paste0("<",Variable," Missing>"),
                            Category),
         Variable = if_else(Variable == "Renal Diagnosis",
                            "Primary Renal Diagnosis",
                            Variable)) %>%
  rename(`SKS (Development)` = res_SKS,`SERPR (Validation)` = res_UoG) %>%
  filter(!is.na(Category)) %>%
  to_kable(caption="Population demographics for the categorical variables presented as number (percent)",align="rr",numeric_cols=2:3,row_names="Category",group_col="Variable")
...
```{r, Comorbidity-Demo, echo=F}
"data/Dev_Paper_Comorbidity_Demographics.csv" %>%
  read_csv(col_types=cols()) %>%
  rename(`SKS (Development)` = res_SKS,`SERPR (Validation)` = res_UoG) %>%
  Dev_Valid_Paper_Var_Names(Variable,Full_Comorb = 2) %>%
  to_kable(caption="Population comorbidity prevalence for the two populations presented as number (percent) <number missing (percent missing)>",
           row_names="Variable",align="rr",numeric_cols=2:3)
...
```{r PH-Three,echo=F}
print_table_PH("data/Dev_Paper_Hazard_Ratios.csv",3)
```
