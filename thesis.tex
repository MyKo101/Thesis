...
The overall accuracy of each model was assessed using the MSM adjusted Brier Score {[}{[}\textbf{Cite: Performance Metrics}{]}{]}, which is a proper score function assigning 0 to a non-informative model and 1 to a perfect model, with negative numbers implying the model performs worse than assuming every patient's state predictions are the same as the overall prevalence within the population.
...
\end{verbatim}
\includegraphics[width=6.24in,height=5.03in,keepaspectratio]{thesis_files/figure-latex/Continuous_Demos-1.png}
...
Within our Multi-State Clinical Prediction Model, {[}\textbf{covariate}{]}, {[}\textbf{covariate}{]} and {[}\textbf{covariate}{]} had the largest effect on the predictions for patient outcomes. This was caused by the strong relationship between the time to {[}\textbf{RRT(?)}{]} and the level of {[}\textbf{covariate}{]}, which also had a strong link to {[}\textbf{mortality(?)}{]}, after adjusting for the other covariates within our model.

{[}\textbf{Describe the baseline hazards, seen in suppl. material}{]}

{[}\textbf{Describe the validation results}{]}
